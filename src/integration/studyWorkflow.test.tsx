 ...
 * "N5 no vocabulary imported"/"N5 4/4 memorized"/etc, and the primary
 * nav bar separately has its own bare "N5" link — both start with the
 * level code, so name-based queries collide. The href is unambiguous.
 */
...
async function flipAndAnswer(result: 'correct' | 'incorrect') {
  fireEvent.click(await screen.findByRole('button', { name: /^flip$/i }))
  const buttonName = result === 'correct' ? /^correct$/i : /^incorrect$/i
...
describe('vocabulary study / flashcard workflow (end-to-end through the UI)', () => {
  it('runs a full study session: setup, flip, correct/incorrect, learning -> memorized, completion, and level isolation', async () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // --- Before flip: only the vocab, no answer key, no way to answer yet. ---
      expect(screen.getByText(vocab)).toBeInTheDocument()
      expect(screen.queryByText(info.meaning)).not.toBeInTheDocument()
      ...
      expect(screen.getByText(info.reading)).toBeInTheDocument()
      expect(screen.getByText(info.meaning)).toBeInTheDocument()
      expect(screen.getByText(info.pos)).toBeInTheDocument()
